...
int main(void)
{
    ...
    if(hostOutput == NULL)
    {
        printf("Host Memory allocation is failed foe hostOutput array.\n");
        cleanup();
        exit(EXIT_FAILURE);
    }
    ...
    if(result != NULL)
    {
        printf("clSetKernel() Failed For 2nd Argument : %d\n",result);
        cleanup();
        exit(EXIT_FAILURE);
    }
    ...
    size_t localWorkSize = 256;  //number of threads
    size_t globalWorkSize;      //number of blocks
    ...
    char str[128];
    if(bAccuracy == false)
        sprintf(str, "Comparison of CPU and GPU Vector Addition is not within accuracy of 0.000001 at array index %d", ibreakValue);
    else
        sprintf(str, "Comparison of CPU and GPU Vector Addition is within accuracy of 0.00001");

    // output
    printf("Array1 begin from 0th index %.6f to %dth index %.6f\n",hostInput1[0], iNumberOfArrayElements - 1, hostInput1[iNumberOfArrayElements - 1]);
    ...
    printf("OpenCL Kernel Global Work Size = %zu and Local Work Size = %zu",globalWorkSize, localWorkSize);
...
E:\CUDA-Seminar-2022\CUDA-Seminar-Assignments\OpenCL\VecAdd>vecAdd.exe
Array1 begin from 0th index 0.001251 to 11444776th index 0.024842
Array2 begins from 0th index 0.891568 to 11444776th index 0.250008
OpenCL Kernel Global Work Size = 11444992 and Local Work Size = 256Output Array begins from 0th index 0.892819 to 11444776th index 0.274850
Time taken for Vector Addition on CPU = 31.635099
Time taken for Vector Addition on GPU = 3.485300
Comparison of CPU and GPU Vector Addition is within accuracy of 0.00001
